...
<body>
  <div id="testheader">
    ...
    <div id="sub_content">
      <form class="row g-3 mt-3 ms-2" th:action="@{/SearchStudent}" method="Post">
        <div class="col-auto">
          <label for="staticEmail2" class="visually-hidden">studentID</label>
          <input type="text"  class="form-control" name="studentid" id="staticEmail2" placeholder="Student ID">
        </div>
        <div class="col-auto">
          <label for="inputPassword2" class="visually-hidden">studentName</label>
          <input type="text" class="form-control" name="studentname" id="inputPassword2" placeholder="Student Name">
        </div>
        <div class="col-auto">
            <label for="inputPassword2" class="visually-hidden">Course</label>
            <input type="text" class="form-control" name="course_name" id="inputPassword2" placeholder="Course">
          </div>
        <div class="col-auto">
...
<div>
      <table class="table table-striped" id="stduentTable">
        ...
        <tbody>
          <tr th:each="stu: ${list}">
            <th scope="row" th:text="${stuStat.index+1}"></th>
            <td th:text="${stu.studentid}"></td>
            <td th:text="${stu.studentname}"></td>
            <td>
            <label th:each="c: ${stu.getCourse()}" th:text="${c.getCourse_name()}+','"></label>
            </td>
            <td>
              <a th:href="@{/setupshowbyid(id=${stu.studentid})}"><button type="submit" class="btn btn-secondary mb-2">See More</button></a> 
            </td>
          </tr>         
        </tbody>
      </table>
